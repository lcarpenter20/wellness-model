...
##Models  
```{r}
df$employment_status <- relevel(factor(df$employment_status), ref = "Not in labor force")
```
...
```{r}
full_model <- glm(wellbeing_response ~ hours_work_wk + residence + employment_status + age, data = df, family = binomial(logit))
summary(full_model)
```

```{r}
## dataset with onyl employed people ----
employed_df <- df %>%
  filter (hours_work_wk != 9999, hourly_wage != 999.99) %>%
  select(wellbeing_response, hours_work_wk, hourly_wage)
```
